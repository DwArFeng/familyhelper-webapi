...
## Release_1.8.2_20260316_build_A
...
- familyhelper-webapi-ui 模块前端依赖升级。
  ...
  - 升级 `prettier` 依赖版本为 `^3.8.1` 以规避漏洞。

- 依赖升级。
  - 升级 `familyhelper-plugin` 依赖版本为 `1.7.1.a` 以应用其新功能。
  ...
  - 升级 `acckeeper` 依赖版本为 `1.9.4.a` 以规避漏洞。
  - 升级 `rbac` 依赖版本为 `1.9.3.a` 以规避漏洞。
  - 升级 `settingrepo` 依赖版本为 `2.4.6.a` 以规避漏洞。
  - 升级 `notify` 依赖版本为 `1.6.3.a` 以规避漏洞。
...
## Release_1.8.1_20251115_build_A
...
- 依赖升级。
  ...
  - 升级 `acckeeper` 依赖版本为 `1.9.2.a` 以规避漏洞。
  - 升级 `rbac` 依赖版本为 `1.9.1.a` 以规避漏洞。
  - 升级 `familyhelper-life` 依赖版本为 `1.2.0.a` 以规避漏洞。
  - 升级 `settingrepo` 依赖版本为 `2.4.1.a` 以规避漏洞。
  - 升级 `notify` 依赖版本为 `1.6.1.a` 以规避漏洞。
...
## Release_1.8.0_20250817_build_A
...
- 依赖升级。
  - 升级 `acckeeper` 依赖版本为 `1.9.1.a` 以规避漏洞。
  - 升级 `rbac` 依赖版本为 `1.9.0.a` 以规避漏洞。
  - 升级 `notify` 依赖版本为 `1.6.0.a` 以规避漏洞。
...
## Release_1.7.0_20250808_build_B
...
## Release_1.7.0_20250808_build_A
...
- `familyhelper-webapi-ui` 模块重构。
  - 使用 pnpm 作为包管理器。
  - 使用 vue3 + vite 等技术栈作为模块的前端框架。
  - 使用 typescript 作为前端的脚本语言。
  - 使用 eslint + prettier 作为代码风格检查工具。
  - 更改模块的打包配置，以适配新的前端框架。
...
- 依赖升级。
  ...
  - 升级 `acckeeper` 依赖版本为 `1.8.2.a` 以规避漏洞。
  - 升级 `rbac` 依赖版本为 `1.8.4.a` 以规避漏洞。
  - 升级 `settingrepo` 依赖版本为 `2.3.2.a` 以规避漏洞。
  - 升级 `notify` 依赖版本为 `1.5.2.a` 以规避漏洞。
