...
import com.dwarfeng.familyhelper.life.sdk.bean.dto.WebInputActivityCoverOrderUpdateInfo;
import com.dwarfeng.familyhelper.life.sdk.bean.entity.JSFixedFastJsonActivityCoverInfo;
...
public class ActivityCoverController {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ActivityCoverController.class);

    private final ActivityCoverResponseService service;

    private final ServiceExceptionMapper sem;

    private final BeanTransformer<ActivityCoverInfo, JSFixedFastJsonActivityCoverInfo> beanTransformer;

    private final CommonsMultipartResolver commonsMultipartResolver;

    private final TokenHandler tokenHandler;

    public ActivityCoverController(
            ActivityCoverResponseService service,
            ServiceExceptionMapper sem,
            BeanTransformer<ActivityCoverInfo, JSFixedFastJsonActivityCoverInfo> beanTransformer,
    ...
    public FastJsonResponseData<JSFixedFastJsonActivityCoverInfo> get(
        ...
        try {
            ActivityCoverInfo activityCoverInfo = service.get(new LongIdKey(longId));
            return FastJsonResponseData.of(ResponseDataUtil.good(
                    JSFixedFastJsonActivityCoverInfo.of(activityCoverInfo)
            ));
        } catch (Exception e) {
            LOGGER.warn("Controller 异常, 信息如下: ", e);
            return FastJsonResponseData.of(ResponseDataUtil.bad(e, sem));
    ...
    public FastJsonResponseData<JSFixedFastJsonPagedData<JSFixedFastJsonActivityCoverInfo>> childForActivity(
        ...
        try {
            PagedData<ActivityCoverInfo> childForActivity = service.childForActivity(
                    new LongIdKey(activityId), new PagingInfo(page, rows)
            );
            PagedData<JSFixedFastJsonActivityCoverInfo> transform = PagingUtil.transform(
    ...
    @GetMapping("/activity-cover/{activityCoverId}/download")
    @BehaviorAnalyse
    @BindingCheck
    @LoginRequired
    public ResponseEntity<Object> download(
        ...
        try {
            ...
            String fileName = adjustFileNameEncoding(activityCover.getOriginName());
            headers.add("Content-Disposition", "attachment;filename=" + fileName);
            body = activityCover.getContent();
        } catch (Exception e) {
    ...
    @PostMapping("/item/{itemId}/activity-cover/upload")
    @BehaviorAnalyse
    @BindingCheck
    @LoginRequired
    public FastJsonResponseData<Object> upload(
        ...
        try {
            ...
            // 将文件内容转换为接口需要的格式，并上传。
            service.upload(
                    accountKey, new ActivityCoverUploadInfo(new LongIdKey(itemId), originFileName, content)
            );
    ...
    @PostMapping("/activity-cover/update-order")
    @BehaviorAnalyse
    @BindingCheck
    @LoginRequired
