...
/**
 * JSFixed FastJson 可展示的登录状态。
 *
 * @author DwArFeng
 * @since 2.0.0
 */
public class JSFixedFastJsonDispLoginState implements Dto {

    private static final long serialVersionUID = -7721927781288157556L;

    public static JSFixedFastJsonDispLoginState of(DispLoginState dispLoginState) {
        if (Objects.isNull(dispLoginState)) {
            return null;
        } else {
            return new JSFixedFastJsonDispLoginState(
                    ...
                    JSFixedFastJsonAccount.of(dispLoginState.getAccount())
            );
        }
    }
    ...
    public JSFixedFastJsonDispLoginState() {
    }

    public JSFixedFastJsonDispLoginState(
            FastJsonStringIdKey key, FastJsonStringIdKey accountKey, Date expireDate, long serialVersion,
            Date generatedDate, int type, String remark, JSFixedFastJsonAccount account
    ) {
        this.key = key;
    ...
    public String toString() {
        return "JSFixedFastJsonDispLoginState{" +
                "key=" + key +
