 ...
 * @since 2.1.0
 */
public class DispExportTask implements Dto {

    private static final long serialVersionUID = -1824593684321413103L;

    public static DispExportTask of(ExportTask exportTask, DispAccount account) {
        if (Objects.isNull(exportTask)) {
            return null;
        } else {
            return new DispExportTask(
                    exportTask.getKey(), exportTask.getSettingKey(), exportTask.getUserKey(),
                    exportTask.getStatus(), exportTask.getCreateDate(), exportTask.getStartDate(),
                    exportTask.getEndDate(), exportTask.getDuration(), exportTask.getDataCount(),
                    exportTask.getMessage(), exportTask.getShouldExpireDate(), exportTask.getShouldDeadDate(),
                    account
            );
        }
    }
